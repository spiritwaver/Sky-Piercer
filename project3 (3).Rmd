...
satfat_mean
for(i in 1:sims){
  satfat_mean[i] <- mean(sample(justfat, n, replace = TRUE) > 22)
}
...
```{r} 
mlb_ttest = t.test(mlb_salary_data$salary ~ mlb_salary_data$NYYorOther, alternative = "greater")
mlb_ttest
```
...
draft_stats <- draft %>%
  ...
  group_by(halfyear) %>%
  summarize(prop = mean(NROW(draft, draft$halfyear == "first")),
            n    = n())
